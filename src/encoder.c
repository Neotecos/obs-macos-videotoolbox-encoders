...
static OSStatus create_encoder(struct vt_encoder *enc)
{
	...
	code = session_set_prop(s, kVTCompressionPropertyKey_RealTime,
				enc->realtime ? kCFBooleanTrue : kCFBooleanFalse);
	if (code != noErr)
		log_osstatus(LOG_WARNING, enc,
...
static void *vt_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	struct vt_encoder_type_data *type_data =
		(struct vt_encoder_type_data *)obs_encoder_get_type_data(encoder);

	OSStatus code;

	enc->encoder = encoder;
	enc->vt_encoder_id = type_data->id;
...
static const char *vt_getname(void *data)
{
	...
	return type_data->plugin_disp_name;
}
...
static void vt_free_type_data(void *data)
{
	...
	bfree((char *)type_data->plugin_disp_name);
	bfree((char *)type_data->id);
	bfree((char *)type_data->obs_id);
	bfree(type_data);
}
...
void obs_module_post_load(void)
{
	...
	for (CFIndex i = 0; i < size; i++) {
		...
		CFBooleanRef hardware_ref =
			CFDictionaryGetValue(encoder_dict, kVTVideoEncoderList_IsHardwareAccelerated);
		bool hardware_accelerated = hardware_ref && CFBooleanGetValue(hardware_ref);
		...
		const char *base_name = disp_name;
		if (codec_type == kCMVideoCodecType_H264) {
			base_name = obs_module_text("VTH264EncHW");
#ifdef ENABLE_HEVC
		} else if (strcmp("Apple HEVC (AVE)", disp_name) == 0) {
			base_name = obs_module_text("VTHEVCEncT2");
		} else if (codec_type == kCMVideoCodecType_HEVC) {
			base_name = obs_module_text("VTHEVCEncHW");
#endif
		}
		...
		type_data->plugin_disp_name = plugin_disp_name;
		type_data->id = id;
		type_data->obs_id = obs_id;
